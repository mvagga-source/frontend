...
function GoodsView() {
    ...
    useEffect(() => {
        ...
        if (res.data.success) setGoods(res.data.goods);
        });*/
    }, [gno]);
    ...
    return (
        ...
        <div className={commonStyles.viewContainer}>
            ...
            <div className={styles.infoBox}>
                ...
                {/* 첫 줄만 제목으로 추출 */}
                <h2 className={styles.goodsName}>{goods.gcontent.split('\n')[0]}</h2>
                ...
                <button 
                    ...
                    disabled={goods.stockCnt === '품절' || goods.status === '숨김' || goods.stockCnt <= 0}
                    onClick={handleOpenModal}
                >
                {goods.status === '품절' || goods.stockCnt <= 0 ? "품절된 상품입니다" : 
                goods.status === '숨김' ? "판매 중단된 상품입니다" : "카카오페이로 구매하기"}
                </button>
                </div>
                {/* 배송 정보 모달 컴포넌트 추가 */}
                <DeliveryModal 
            ...
            {activeTab === "detail" && (
                ...
                {/* 제목 제외한 나머지 본문 표시 */}
                {goods.gcontent.split('\n').slice(1).map((line, idx) => (
                    <p key={idx}>{line}</p>
                ))}
                <div className={styles.dummyImagePlaceholder}>
                    <img src={goods.gimg} alt="상세이미지" style={{width: '100%', marginTop: '20px', borderRadius: '8px'}} />
                </div>
                </div>
            )}
            ...
            {user && user.id === goods.member?.id && (
                ...
                <DelBtn onClick={() => { if(window.confirm("삭제하시겠습니까?")) alert("삭제 처리"); }}>삭제</DelBtn>
                </div>
            )}
            </div>
